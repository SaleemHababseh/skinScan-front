  ...
  loadAdminData: async () => {
    ...
    try {
      ...
      const stats = {
        ...
        activeDoctors: users.filter(user => user.role === 'doctor' && user.status === 'active').length,
      ...
      const recentActivities = logs
        ...
        .map(log => ({
          ...
          type: log.action.toLowerCase().includes('user') ? 'user' : 
                log.action.toLowerCase().includes('diagnosis') ? 'diagnosis' :
                log.action.toLowerCase().includes('system') ? 'system' : 'other',
